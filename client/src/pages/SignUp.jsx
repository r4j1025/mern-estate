...
const SignUp = () => {
  ...
  const handleSubmit= async (e)=>{
    ...
    try {
      setLoading(true);
      const res = await fetch('api/auth/signup',     //proxy was added in vite.config.js
      {
        method:'POST',
        headers:{
      ...
      setLoading(false); //loading is completed after execution of fetch function
      setError(null);
      navigate('/sign-in');
    } catch (error) {
      setLoading(false);
  ...
  return (
    <div className="p-3 max-w-lg mx-auto">
      ...
      {error && <p className="to-red-500 mt-5" >{error}</p>}
    </div>
  );
};
