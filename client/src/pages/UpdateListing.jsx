...
const CreateListing = () => {
    ...
    useEffect(() => {    // we cannot call async here.
      const fetchListing = async () =>{
        const listingId = params.listingId  // this name was given in app.jsx
        ...
        setFormData(data);  // setting the data recieved from api to the formdata.
      }
      fetchListing();
    }, [])
    ...
    const handleChange = (e) =>{
        ...
        if (e.target.id === 'parking' || e.target.id === 'furnished' || e.target.id === 'offer' ){
            setFormData({
                ...formData,
                [e.target.id] : e.target.checked, //if we dont add [] then it will be "parking" instead of parking (without quotes).
            })
        }
        if (e.target.type === 'number' || e.target.type === 'text'||e.target.type === 'textarea'){ //mongdb will automatically convert the datatype so no need to worry eg : "1000" to 1000.
            setFormData({
                ...formData,
    ...
    const handleSubmit = async (e)=>{
      ...
      if (+formData.regularPrice < +formData.discountPrice) //adding '+' making them as a number.
    ...
    <main className='p-3 max-w-4xl mx-auto'>
        ...
        <form onSubmit={handleSubmit} className='flex gap-4 flex-col sm:flex-row'>
            <div className='flex flex-col gap-4 flex-1'>
                <input type='text' onChange={handleChange} value={formData.name} id='name' maxLength='62' required minLength='10' placeholder='Name' className='border p-3 rounded-lg'/>
                <textarea type='text' id='description' onChange={handleChange} value={formData.description} maxLength='300' required  placeholder='Description' className='border p-3 rounded-lg'/>
                <input type='text' id='address' onChange={handleChange} value={formData.address} required placeholder='Address' className='border p-3 rounded-lg'/>
                ...
                <div className='flex items-center gap-2'>
                    <input className='p-3 border border-gray-300 rounded-lg' type='number' id='bedrooms' onChange={handleChange} value={formData.bedrooms} min='1' max='10' required  />
                    <p>Bedrooms</p>
                </div>
                <div className='flex items-center gap-2'>
                    <input className='p-3 border border-gray-300 rounded-lg' type='number' id='bathrooms' onChange={handleChange} value={formData.bathrooms} min='1' max='10' required  />
                    <p>Bathrooms</p>
                </div>
                <div className='flex items-center gap-2'>
                    <input className='p-3 border border-gray-300 rounded-lg' type='number' id='regularPrice' onChange={handleChange} value={formData.regularPrice} min='1000' max='10000000' required  />
                ...
                { formData.offer &&
                <div className='flex items-center gap-2'>
                    <input className='p-3 border border-gray-300 rounded-lg' type='number' id='discountPrice' onChange={handleChange} value={formData.discountPrice} min='0' max='10000000' required  />
                    <div className='flex flex-col items-center'>
                    <p>Discount price</p>
                    <span className='text-xs' >(₹ / Month)</span>
                    </div>
            ...
            <div className='flex flex-col gap-4 flex-1'>
                ...
                <button disabled={loading || uploading} className='text-white p-3 rounded-lg disabled:opacity-80 hover:opacity-80 bg-slate-700 uppercase' >{loading ? "Updating...":" Update Listing"}</button>
